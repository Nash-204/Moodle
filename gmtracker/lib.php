...
/**
 * Queue training invitations for background processing
 */
function gmtracker_queue_training_invites($gmtrackerid, $userid) {
    global $DB, $CFG;
    
    // Get gmtracker and course for logging
    $gmtracker = $DB->get_record('gmtracker', ['id' => $gmtrackerid]);
    $course = $DB->get_record('course', ['id' => $gmtracker->course]);
    ...
    $task = new \mod_gmtracker\task\send_training_invites_task();
    $task->set_custom_data([
        'gmtrackerid' => $gmtrackerid,
        'sentby' => $userid,
        'batchsize' => 25
    ]);
    ...
    $params = [
        'context' => $context,
        'objectid' => $gmtrackerid,
        'userid' => $userid,
        'other' => [
            'gmtrackername' => $gmtracker->name,
            'totalusers' => $totalusers
        ]
    ];
    
    $event = \mod_gmtracker\event\training_invite_queued::create($params);
    $event->trigger();
}

/**
 * Log training invitation sent to individual user
 */
function gmtracker_log_invite_sent($gmtracker, $course, $sentby, $recipientid) {
    $context = \context_course::instance($course->id);
    $params = [
        'context' => $context,
    ...
    $event = \mod_gmtracker\event\training_invite_sent::create($params);
    $event->trigger();
}

/**
 * Log training invitation failure for individual user
 */
function gmtracker_log_invite_failed($gmtracker, $course, $sentby, $recipientid, $error) {
    $context = \context_course::instance($course->id);
    $params = [
        'context' => $context,
        'objectid' => $gmtracker->id,
    ...
    $event = \mod_gmtracker\event\training_invite_failed::create($params);
    $event->trigger();
}

/**
 * Log bulk training invitation start
 */
function gmtracker_log_invite_bulk_start($gmtracker, $course, $sentby, $totalusers) {
    $context = \context_course::instance($course->id);
    $params = [
        'context' => $context,
        'objectid' => $gmtracker->id,
    ...
    $event = \mod_gmtracker\event\training_invite_bulk_start::create($params);
    $event->trigger();
}

/**
 * Log bulk training invitation completion
 */
function gmtracker_log_invite_bulk_complete($gmtracker, $course, $sentby, $sentcount, $totalusers, $failedemails) {
    $context = \context_course::instance($course->id);
    $params = [
        'context' => $context,
        'objectid' => $gmtracker->id,
    ...
    $event = \mod_gmtracker\event\training_invite_bulk_complete::create($params);
    $event->trigger();
}
...
function gmtracker_add_instance($data, $mform = null) {
    ...
    // Email notifications for BOTH online and onsite meetings - NOW IN BACKGROUND
    if (!get_config('gmtracker', 'sendemailnotifications')) {
        return $data->id;
    }

    // Queue background task for email sending
    gmtracker_queue_training_invites($data->id, $USER->id);
