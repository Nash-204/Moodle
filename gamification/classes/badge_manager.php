...
class badge_manager {
     ...
     * @param string|null $period Specific period (e.g., "2024-03", "2024")
     * @param string|null $displayName Custom display name
     * @return bool true if awarded, false if already exists
     */
    ...
    public static function get_user_badges(int $userid = 0): array {
        ...
        foreach ($allBadges as $badgeDef) {
            // Skip if this is already in output as a local badge
        ...
        // Finally, add unearned badges that aren't already in output
        foreach ($allBadges as $badgeDef) {
            $alreadyInOutput = false;
            foreach ($regularBadges as $outputBadge) {
                if ($outputBadge->badgecode === $badgeDef->badgecode) {
        ...
        // Combine: period badges first, then regular badges
        return array_merge($periodBadges, $regularBadges);
    }
    ...
    public static function get_all_badge_definitions_with_moodle(): array {
        ...
        foreach ($localBadges as $badge) {
            ...
            $badge->display_name = $badge->name; // Set default display_name
            $allBadges[] = $badge;
        }
     ...
     * This includes badges from courses AND site-level badges (where courseid is NULL).
     */
    private static function get_all_moodle_badges(): array {
        ...
        $sql = "SELECT b.id, b.name, b.description, b.timecreated, b.timemodified,
                ...
                WHERE b.status != 0  -- Only active badges (not deleted/draft)
                ORDER BY 
                  CASE WHEN b.courseid IS NULL THEN 0 ELSE 1 END, -- Site badges first
                  b.courseid, 
                  b.name";
    ...
    /**
     * Get Moodle core badges earned by a user (both course and site-level).
     */
    ...
    /**
     * Get Moodle badge by code (both course and site-level).
     */
    ...
    /**
     * Check if a user has a badge (both local and Moodle core).
     */
    ...
    /**
     * Return array of local badge definitions (helper).
     */
