...
class block_gamification_external_badges extends external_api {
    ...
    public static function get_user_badges_by_email($email) {
        ...
        // Hash the email for security before any processing
        $emailHash = self::hash_email($params['email']);
        ...
        try {
            ...
            foreach ($allEarnedBadges as $badge) {
                $formatted[] = [
                    ...
                    'name' => $badge->display_name ?? $badge->name ?? 'Unknown Badge',
