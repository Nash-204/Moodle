...
class observer {
    ...
    public static function user_loggedin(\core\event\user_loggedin $event): void {
        ...
        try {
            ...
            $userLastWeek = clone $userTodayStart;
            $userLastWeek->modify('-7 days');
            $lastweekts = $userLastWeek->getTimestamp();
            ...
            // WEEKLY STREAK - Only count weeks where user logged in at least once on a weekday
            $lastweeklycheck = (int)$record->lastweeklycheck;
            if (!$isWeekend && $lastlogin >= $lastweekts && $lastweeklycheck < $todayts) {
                // User logged in this week on a weekday and weekly streak not updated yet
                $daysSinceLastCheck = floor(($todayts - $lastweeklycheck) / (60 * 60 * 24));
                if ($daysSinceLastCheck >= 7) {
                    $record->weeklystreak += 1;
                    $record->lastweeklycheck = $todayts;
                }
            } else if (!$isWeekend && $lastlogin < $lastweekts) {
                // Missed a week (no weekday login)
                $record->weeklystreak = 1;
                $record->lastweeklycheck = $todayts;
            }
            ...
            if (!$isWeekend) {
                ...
                if ($record->weeklystreak >= 4) { // 4 consecutive weeks with weekday login
